...
$sql = "SELECT tblcars.*,tblbrands.BrandName,tblbrands.id as bid  from tblcars join tblbrands on tblbrands.id=tblcars.VehiclesBrand where tblcars.id=:vhid";
$query = $dbh -> prepare($sql);
$query->bindParam(':vhid',$vhid, PDO::PARAM_STR);
$query->execute();
...
<?php 
$bid=$_SESSION['brndid'];
$sql="SELECT tblcars.VehiclesTitle,tblbrands.BrandName,tblcars.PricePerDay,tblcars.FuelType,tblcars.ModelYear,tblcars.id,tblcars.SeatingCapacity,tblcars.VehiclesOverview,tblcars.Vimage1 from tblcars join tblbrands on tblbrands.id=tblcars.VehiclesBrand where tblcars.VehiclesBrand=:bid";
$query = $dbh -> prepare($sql);
